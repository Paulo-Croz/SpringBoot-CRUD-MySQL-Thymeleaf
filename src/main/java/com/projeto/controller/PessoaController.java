...
import com.projeto.repository.Pessoas;
...
public class PessoaController {
	...
	@RequestMapping(method= RequestMethod.GET, value="salvarpessoa") //
	...
	public ModelAndView addFonePessoa(Telefone telefone , 
		...
		ModelAndView mv1 = new ModelAndView("telefones");

		telefone.setPessoa(pessoa);
		
		telefoneRepository.save(telefone);
		
		mv1.addObject("pessoa", pessoa);
		mv1.addObject("telefones", telefoneRepository.getTelefones(pessoaid));
		return mv1;
	}
	@RequestMapping("/removertelefone/{pessoaid}")
	public ModelAndView removertelefone(Telefone telefone , 
									 @PathVariable("pessoaid") Long pessoaid) {
		
		Pessoa pessoa = pessoas.findById(pessoaid).get();
		
		
		ModelAndView mv2 = new ModelAndView("telefones");

		telefone.setPessoa(pessoa);
		
		telefoneRepository.delete(telefone);
		
		mv2.addObject("pessoa", pessoa);
		mv2.addObject("telefones", telefoneRepository.getTelefones(pessoaid));
		return mv2;
	}
}
